...
func TestStructIndexes(t *testing.T) {
	type C struct {
		// Deliberate fixture: the unexported field is what shifts the field
		// indexes, which is the whole point of this test (mixed exported and
		// unexported fields must not panic). Removing it voids the test.
		//lint:ignore U1000 load-bearing test fixture for mixed field indexes
		something int
...
func TestTagWithStringOption(t *testing.T) {
	...
	type Address struct {
		...
		// Deliberate fixture: `,string` on a struct pointer is exactly the case
		// under test (this package honours the option via fmt.Stringer, which
		// encoding/json does not). The "invalid" tag is the input, not a defect.
		//lint:ignore SA5008 load-bearing test fixture for the ,string tag option
...
type Dog struct {
	// Deliberate fixture: *Animal is deliberately NOT a fmt.Stringer, so this
	// exercises the ,string fallback path. The "invalid" tag is the input.
	//lint:ignore SA5008 load-bearing test fixture for the ,string tag option
